...
def generate_trunk_config(intf_vlan_mapping, trunk_template):
    ...
    ВСЕ ЗАПИСЫВАЕТСЯ В ФАЙЛ result_9_2_print.txt
    """
    f = open('result_9_2_print.txt', 'w')
    for intf, value in intf_vlan_mapping.items():
        print("interface" + intf, file = f)
        for command in trunk_template:
            if command.endswith("allowed vlan"):
                list_vlan_str = list(map(str, value))
                vlans = ','.join(list_vlan_str)
                print(f" {command} {vlans}", file = f)
            else:
