...
def generate_access_config(intf_vlan_mapping, access_template, port_security=None):
    ...
    port_security_template - cписок команд для порта c настройками port_security
    ...
    for intf, vlan in intf_vlan_mapping.items():
        ...
        result.append(command_intf)
        for command in access_template:
            if command.endswith("vlan"):
                command_vlan = f" {command} {vlan}"
                result.append(command_vlan)
            else:
                command_another = f" {command}"
